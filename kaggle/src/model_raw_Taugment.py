...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    rd_factor = max(min(np.random.normal(0.8,0.2,size=(1,1)),1),0.6)
    scale_factor = float(rd_factor)
    #print(scale_factor)
    h,w = image.shape[:2]
    if augment:
        if scale_factor < 1:
    	    image,mask = transform.random_crop_transform2(image, mask, int(w*scale_factor),int(h*scale_factor), u=1)
...
class nms_layer(KE.Layer):

    def __init__(self,config,proposal_count,**kwargs):
       self.config=config
       self.proposal_count=proposal_count
       super(nms_layer, self).__init__(**kwargs)

    def call(self,inputs): 
        normalized_boxes = inputs[0]
        scores = inputs[1]
        """
        indices = tf.image.non_max_suppression(
                                         normalized_boxes, scores, self.proposal_count,
...
class RawproposalLayer(KE.Layer):
    """Receives anchor scores and selects a subset to pass as proposals
    to the second stage. Filtering is done based on anchor scores and
    non-max suppression to remove overlaps. It also applies bounding
    box refinement deltas to anchors.
    ...
    def __init__(self, proposal_count, nms_threshold, anchors,flag=None,
                 config=None, **kwargs):
        """
        anchors: [N, (y1, x1, y2, x2)] anchors defined in image coordinates
        """
        super(RawproposalLayer, self).__init__(**kwargs)
        self.config = config
        self.proposal_count = proposal_count
        self.nms_threshold = nms_threshold
        self.anchors = anchors.astype(np.float32)
        self.flag = config.MAIN_SCALE
    ...
    def call(self, inputs):
        ...
        # Clip to image boundaries. [batch, N, (y1, x1, y2, x2)]
        height, width = self.config.SCALES[self.flag],self.config.SCALES[self.flag]
        window = np.array([0, 0, height, width]).astype(np.float32)
        boxes = utils.batch_slice(boxes,
                                  lambda x: clip_boxes_graph(x, window),
                                  self.config.IMAGES_PER_GPU,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        normalized_boxes,scores = RawproposalLayer(proposal_count=proposal_count,
                                 nms_threshold=config.RPN_NMS_THRESHOLD,
                                 name="ROI",
                                 anchors=self.anchors,
                                 config=config)([rpn_class, rpn_bbox])

        rpn_rois = nms_layer(config, 2000)([normalized_boxes,scores])
        if mode == "training":
